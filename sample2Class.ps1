<#
	Class はスクリプト スコープなので、継承した Class (TelnetAllied) はこのファイルの末尾に書いてある
#>
...
##############################################
# TelnetClient class の継承実装サンプル
##############################################
# 特定メーカースイッチ用 telnet class
class TelnetAllied : TelnetClient {
	...
	[void] Logon( [string] $Password ){
		...
		# ネゴシエーション
		$ReceiveStrings = ([TelnetClient]$this).Receive( "login:" )

		# ID 送信
		([TelnetClient]$this).Send( "admin", $true )

		# パスワードプロンプトを待つ
		$ReceiveStrings = ([TelnetClient]$this).Receive( "Password:" )

		# パスワード送信
		([TelnetClient]$this).Send( $Password, $false )

		# プロンプトを待つ
		$ReceiveStrings = ([TelnetClient]$this).Receive( ">" )

		# モード変更
		([TelnetClient]$this).Send( "enable", $true )
	...
	[void] Logoff(){
		# コマンド送信
		([TelnetClient]$this).Send( "disable", $true )

		# プロンプトを待つ
		$ReceiveStrings = ([TelnetClient]$this).Receive( ">" )

		# logoff コマンド送信
		([TelnetClient]$this).Send( "exit", $true )
	...
	[string[]] SendCommand( [string] $Command ){
		...
		# コマンド送信
		([TelnetClient]$this).Send( $Command, $true)

		# プロンプトを待つ
		[string[]] $ReceiveStrings = ([TelnetClient]$this).Receive( "#" )
